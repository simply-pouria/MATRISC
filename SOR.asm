...
matrix:
    .float  10,  -1,  2,  0,    # row 0
           ...
           0, 3, -1, 8     # row 3
omega:
    .float  0.8  # Relaxation factor (try values between 0 and 2)
one_minus_omega:
    .float  0.0   # Will be calculated as 1 - omega
zero_f:
    .float  0.0
str_row:
    .string "Row "
...
main:
    ...
    # Calculate 1 - omega
    la    t0, omega
    flw   f1, 0(t0)        # f1 = omega
    li    t1, 1
    fcvt.s.w f2, t1        # f2 = 1.0
    fsub.s f3, f2, f1      # f3 = 1 - omega
    la    t0, one_minus_omega
    fsw   f3, 0(t0)        # store 1 - omega
...
row_loop:
    ...
    # --- get current x_i (old value) ---
    mv    a0, s1           # vector_x base
    mv    a1, s6           # index i
    jal   get_vec_elem
    fmv.s f9, f0           # f9 = x_i_old

    # --- compute dot product (skip diagonal element i) ---
    # Use current vector_x (for j < i, new; for j > i, old)
    ...
    # --- compute residual: (b[i] - dot) / a_ii ---
    fsub.s f0, f7, f6      # f0 = b[i] - dot
    fdiv.s f0, f0, f8      # f0 = residual = (b[i] - dot) / a_ii

    # --- load omega and 1 - omega ---
    la    t0, omega
    flw   f10, 0(t0)       # f10 = omega
    la    t0, one_minus_omega
    flw   f11, 0(t0)       # f11 = 1 - omega

    # --- compute x_i_new = (1 - omega) * x_i_old + omega * residual ---
    fmul.s f12, f11, f9    # f12 = (1 - omega) * x_i_old
    fmul.s f13, f10, f0    # f13 = omega * residual
    fadd.s f0, f12, f13    # f0 = x_i_new
